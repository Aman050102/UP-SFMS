...
def api_check_in(request):
    ...
    try:
        res = supabase.rpc(
            "rpc_scan_check_in",
            {"p_email": email, "p_plain_code": pin, "p_facility_name": facility},
        ).execute()
...
def api_admin_set_pin(request):
    if supabase_service is None:
        return JsonResponse(
            {"ok": False, "error": "SERVICE_KEY_NOT_CONFIGURED"}, status=500
        )
    body = _json(request)
    email = body.get("email")
    pin = body.get("pin")
    if not (email and pin):
        return JsonResponse({"ok": False, "error": "MISSING_FIELDS"}, status=400)
    try:
        res = supabase_service.rpc(
            "admin_set_scan_code", {"p_email": email, "p_plain_code": pin}
        ).execute()
        return JsonResponse({"ok": True, "data": res.data}, status=200)
    except Exception as e:
        return JsonResponse({"ok": False, "error": str(e)}, status=400)
